...
- **SPINT**（Le et al., NeurIPS 2025）：提出通过参考窗口原始神经活动（binned spike counts）生成 unit embedding 的 IDEncoder 思路，实现梯度无关跨 session 泛化；NeuroHorizon IDEncoder 模块的直接灵感来源，跨 session 泛化设计应与其对比
