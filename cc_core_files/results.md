...
- **产生方式**：
  ...
  - `examples/neurohorizon/train.py --config-name=train_1p10_latent_dynamics_mamba_500ms epochs=1 eval_epochs=1 batch_size=16 eval_batch_size=16 num_workers=0`
  - `scripts/analysis/neurohorizon/eval_phase1_v2.py --checkpoint-kind best --split valid`
- **实验目的**：
  - 验证 `Mamba` backbone 是否已打通最小训练与离线评估链路
  - 判断在缺少 `mamba-ssm/causal-conv1d` kernels 时，`transformers + mambapy` fallback 是否足以支撑 smoke
- **实验配置**：
  - 模型：Mamba latent dynamics decoder
  - backend：`transformers_mambapy`
  - 数据：Perich-Miller 10 sessions
  - 协议：continuous，obs=500ms，pred=500ms
  - 训练：1 epoch smoke，`batch_size=16`
- **主要结果**：

  | run | backend | train loss | val loss | valid fp-bps | valid R2 |
  |-----|---------|------------|----------|--------------|----------|
  | `20260322_latent_dynamics_mamba_gate` smoke | `transformers_mambapy` | 0.415 | 0.395 | -0.8285 | 0.0001 |

- **结果分析**：
  - 这一轮 smoke 已经证明：`Mamba` 方向在当前仓库中可以最小落地，`forward/generate` 一致性、训练、checkpoint 和离线 valid eval 链路都已跑通
  - 但当前使用的是 `transformers + mambapy` fallback，性能和显存效率都明显不足
  - 直接用 fallback 跑原始 smoke 配置 `batch_size=128` 会 OOM，只能把 batch 下调到 `16`
  - 因此当前结论不是“方向失败”，而是“正式 gate 仍需要更高效的 kernel backend”
- **备注**：
  - verify 输出：`mamba_backend=transformers_mambapy`, `output_shape=(2, 25, 6)`, `tf_vs_rollout_max_delta=0.000000`
  - `mamba-ssm` / `causal-conv1d` 仍在环境侧排查中，本轮暂未启动 formal `500ms` gate
