...
## Phase 1.3.4 Benchmark 对比：NeuroHorizon vs NDT2 / Neuroformer / IBL-MtM

> 记录：引用 1.8.3 benchmark 结果，条件完全一致（10 sessions、500ms obs_window、连续训练、同一数据划分）
> 执行日期：2026-03-12
...
1. **NeuroHorizon 在所有预测窗口上 fp-bps 最优**，且参数量最小（~2.1M vs 4.8-10.7M）
2. **Neuroformer 是最强竞争者**（decoder-only GPT 架构，逐 spike 自回归生成），250ms 差距 14%
3. **NDT2 和 IBL-MtM 差距较大**（25% 和 21%），并行预测方式在长时程任务上不如自回归
4. **NeuroHorizon 优势随窗口增大变化不一**：vs Neuroformer 优势缩小（14%→9%），vs IBL-MtM 优势增大（21%→32%）
...
- **当前结论**：
  - 与旧 `limited-window debug e1` 相比，IBL-MtM full-data 250ms 已明显改善，但仍显著为负
  - 当前可以确认：IBL 的训练语义已经恢复为 upstream multi-mask，而不是此前的 fixed `forward-pred` 训练

### Neuroformer faithful dual-mode smoke v2 与 250ms formal runtime blocker（2026-03-18）

- **双模式 smoke 路径**：`results/logs/phase1_benchmark_faithful_neuroformer_smoke_dualmode_v2/`
- **双模式 smoke 关键结果**：
  - valid rollout `fp-bps = -13.8295`
  - valid true_past `fp-bps = -10.9855`
  - trial rollout / true_past `fp-bps = -10.8324 / -10.8523`
- **关键修正**：
  - 正式支持 `rollout(true_past=False)` 与 `true_past=True`
  - `true_past` 改为直接复用 teacher-forced 前向输出解码，不再逐 token 重跑模型
- **250ms formal 运行成本审计**：
  - `phase1_benchmark_repro_faithful_neuroformer_250ms_dualmode_e1` 训练后进入 dual-mode held-out generation，累计超过 `30 min` 仍未产出最终 `results.json`
  - test 窗口真实 event 统计：`mean=64.0`, `p95=113`, `p99=134.9`, `max=167`
  - 按此收紧到 `max_generate_steps=192` 再次尝试 `phase1_benchmark_repro_faithful_neuroformer_250ms_dualmode_e1_g192`，`13+ min` 时仍未到 checkpoint
- **当前结论**：
  - Neuroformer 的推理语义已经对齐，但 250ms formal full-data dual-mode eval 目前先被运行成本卡住
  - 因此 500ms / 1000ms 不能继续，必须先解决 250ms 的执行成本问题
