...
## Phase 1.10 Latent Dynamics 结果

### 20260320_latent_dynamics_decoder — 250ms smoke run

- **存储路径**：`results/logs/1.10-latent_dynamics_decoder/20260320_latent_dynamics_decoder/250ms_smoke/`
- **产生时间**：2026-03-20
- **产生方式**：`scripts/1.10-latent_dynamics_decoder/20260320_latent_dynamics_decoder/run_latent_dynamics_smoke.sh`
- **实验目的**：验证 `1.10` latent-dynamics 主线的训练、checkpoint 和离线 continuous eval 链路是否打通
- **实验配置**：
  - 模型：GRU latent dynamics decoder
  - 数据：Perich-Miller 10 sessions
  - 协议：continuous，obs=500ms，pred=250ms
  - override：`epochs=1 eval_epochs=1 batch_size=256 eval_batch_size=256 num_workers=0`
- **主要结果**：
  - train loss：`0.417`
  - val loss：`0.406`
  - train-end `val/fp_bps`：`-0.834`
  - 离线 continuous valid：`fp-bps=-0.8339`, `R2=-0.0021`, `val_loss=0.4079`
- **结果分析**：
  - 该 smoke run 已证明 `latent_dynamics` 分支可以正常完成训练、保存 `best.ckpt`、再执行离线评估
  - 1 epoch 下指标为负是预期现象，当前结果仅用于验证链路，不用于判断该方向有效性
- **备注**：
  - 对应功能验证脚本：`verify_latent_dynamics.py`
  - 正式三窗口结果见下节

### 20260320_latent_dynamics_decoder — 正式三窗口结果

- **存储路径**：
  - `results/logs/1.10-latent_dynamics_decoder/20260320_latent_dynamics_decoder/{250ms,500ms,1000ms}/`
...
- **产生时间**：2026-03-20
- **产生方式**：
  - `scripts/1.10-latent_dynamics_decoder/20260320_latent_dynamics_decoder/run_latent_dynamics_experiments.sh`
  - `scripts/analysis/neurohorizon/eval_phase1_v2.py`
  - `scripts/1.10-latent_dynamics_decoder/20260320_latent_dynamics_decoder/collect_latent_dynamics_results.py`
  - `scripts/1.10-latent_dynamics_decoder/plot_optimization_training_curves.py --module 20260320_latent_dynamics_decoder`
  - `scripts/1.10-latent_dynamics_decoder/plot_optimization_progress.py`
- **实验目的**：
  - 给 `1.10` 的首个 GRU latent dynamics baseline 产出可直接和 `baseline_v2` 对比的三窗口正式结果
  - 判断在移除 observation-space feedback 并统一成 latent rollout 后，是否能在更长 horizon 上取得收益
- **实验配置**：
  - 模型：GRU latent dynamics decoder
  - 数据：Perich-Miller 10 sessions
  - 协议：continuous，obs=500ms，pred=250ms / 500ms / 1000ms
  - 训练：300 epochs
  - 说明：该实现中 `teacher-forced == rollout`
- **主要结果**：

  | pred_window | valid fp-bps | test fp-bps | valid R2 | test R2 | baseline_v2 | valid delta | test delta |
  |-------------|--------------|-------------|----------|---------|-------------|-------------|------------|
  | 250ms | 0.1882 | 0.1966 | 0.2493 | 0.2511 | 0.2115 | -0.0233 | -0.0149 |
  | 500ms | 0.0904 | 0.0857 | 0.2041 | 0.2033 | 0.1744 | -0.0840 | -0.0887 |
  | 1000ms | 0.0674 | 0.0667 | 0.1946 | 0.1937 | 0.1317 | -0.0643 | -0.0650 |

- **图表与解读**：
  - `training_curves.png`
    - 该图对应三个窗口的 `train_loss / val_loss / val/fp_bps` 曲线
    - `250ms` 的 best checkpoint 出现在 `epoch 289`，`epoch 299` 的 final 指标几乎一致，说明该窗口已基本收敛到 `约0.188 fp-bps`
    - `500ms` 的 best checkpoint 出现在 `epoch 259`，到 `epoch 299` 反而从 `0.0902` 回落到 `0.0828`，说明问题不是“没训练够”，而是当前 GRU latent dynamics 在中长 horizon 上的上限较低
    - `1000ms` 的 best checkpoint 出现在 `epoch 289`，final 与 best 仅相差 `约0.001`，说明它同样已基本稳定，但稳定在一个明显低于 `baseline_v2` 的水平
  - `optimization_progress.png`
    - 该图当前只有一个 `1.10` 实验点，作用主要是把 `20260320_latent_dynamics_decoder` 设为新的 latent-dynamics baseline
    - 图中可以直接看到：`250ms` 仅略低于 `baseline_v2`，而 `500ms / 1000ms` 与 baseline 的差距显著更大
  - `latent_dynamics_summary.json`
    - 该 summary 把三个窗口的 best checkpoint、continuous valid 结果和 per-bin `fp-bps` 固化下来
    - 由于当前 decoder 把 `forward()` 与 `generate()` 统一到同一 latent rollout 路径，summary 同时验证了本轮不存在 teacher-forced / rollout 的语义差异；因此结果不足更像 latent representation / dynamics 建模能力不足，而不是 exposure bias
- **结果分析**：
  - 这次首轮 GRU latent dynamics baseline 已经证明：方向切换在工程上是成立的，训练、best ckpt、离线 `valid/test`、结果汇总、趋势图和训练曲线都已打通
  - 但从效果上看，它没有兑现“latent dynamics 在长 horizon 上应优于 baseline_v2”的预期
  - `250ms` 已经接近 `baseline_v2`，说明 latent rollout 并非完全失效
  - `500ms / 1000ms` 明显落后，说明当前的 attention pooling 压缩、latent state 容量和 autonomous GRU dynamics 还不够强
  - 因为本实现里 `teacher-forced == rollout`，所以本轮结果也给出了一个更清晰的负结论：当前瓶颈不在 rollout gap，而在 latent dynamics 自身的表达能力
- **备注**：
  - 本轮实现 commit：`32a1f5f`
  - 结果已同步写入 `cc_todo/1.10-latent_dynamics_decoder/results.tsv`
  - 下一轮 `1.10.x` 应优先尝试更大的 latent state 或更强的 dynamics backbone，而不是继续在当前 GRU baseline 上做小幅 regularization 调整

### 20260320_latent_dynamics_state_scaling — 500ms gate

- **存储路径**：
  - `results/logs/1.10-latent_dynamics_decoder/20260320_latent_dynamics_state_scaling/500ms/`
  ...
  - `results/figures/1.10-latent_dynamics_decoder/20260320_latent_dynamics_state_scaling/training_curves.{png,pdf}`
  - `results/figures/1.10-latent_dynamics_decoder/optimization_progress.{png,pdf}`
- **产生时间**：2026-03-21
- **产生方式**：
  - `scripts/1.10-latent_dynamics_decoder/20260320_latent_dynamics_state_scaling/run_latent_dynamics_state_scaling_500ms.sh`
  - `scripts/analysis/neurohorizon/eval_phase1_v2.py`
  - `scripts/1.10-latent_dynamics_decoder/20260320_latent_dynamics_state_scaling/collect_latent_dynamics_state_scaling_results.py`
  - `scripts/1.10-latent_dynamics_decoder/plot_optimization_training_curves.py --module 20260320_latent_dynamics_state_scaling`
  - `scripts/1.10-latent_dynamics_decoder/plot_optimization_progress.py`
- **实验目的**：
  - 验证“放大 latent state 容量”是否能显著改善上一轮 `500ms valid fp-bps = 0.0904`
  - 若 `500ms` gate 无明显提升，则直接终止该方向，不再扩展 `250ms / 1000ms`
- **实验配置**：
  - 模型：GRU latent dynamics decoder
  - 改动：`num_pool_tokens=8`, `pool_token_dim=64`, `state_dim=256`
  - 数据：Perich-Miller 10 sessions
  - 协议：continuous，obs=500ms，pred=500ms
  - 训练：300 epochs
  - 说明：本模块只做 `500ms gate`
- **主要结果**：

  | run | valid fp-bps | test fp-bps | valid R2 | test R2 | baseline_v2 | vs 上一轮 500ms |
  |-----|--------------|-------------|----------|---------|-------------|-----------------|
  | `20260320_latent_dynamics_state_scaling` | 0.0048 | 0.0049 | 0.1791 | 0.1790 | 0.1744 | -0.0856 |

- **图表与解读**：
  - `training_curves.png`
    - best checkpoint 出现在 `epoch 69`，对应 best valid `fp-bps=0.0048`
    - 到 `epoch 299` 时 `val/fp_bps` 已回落到 `0.0006`，说明这轮不是“训练太短”，而是更大 state 在当前 GRU 结构下没有形成稳定增益
  - `optimization_progress.png`
    - 新增的 `20260320_latent_dynamics_state_scaling` 行只在 `500ms` 有值
    - 图上可以直接看到它相对 `20260320_latent_dynamics_decoder` 的 `500ms=0.0904` 出现大幅回退
  - `latent_dynamics_state_scaling_summary.json`
    - 固化了本轮 `500ms` gate 的 best checkpoint 与 formal valid 指标
    - 便于后续训练曲线脚本和 `results.tsv` 统一读取，不再只依赖 screen log 人工回看
- **结果分析**：
  - 这轮 gate 已经给出一个明确的负结论：单纯放大 latent state 容量并不能解决 `500ms` 的问题
  - 相反，它把 `500ms valid fp-bps` 从上一轮的 `0.0904` 拉低到了 `0.0048`
  - 因此当前瓶颈不太像“state 太小”，更像 dynamics 结构、context 注入方式或优化稳定性本身存在问题
- **备注**：
  - 本轮实现 commit：`ef0f6fd`
  - 该模块已标记为“已放弃”，不再扩展到 `250ms / 1000ms`
  - 下一轮 `1.10.x` 更适合转向更强的 dynamics backbone 或显式 context skip

### 20260321_latent_dynamics_context_skip — 500ms gate

- **存储路径**：
  - `results/logs/1.10-latent_dynamics_decoder/20260321_latent_dynamics_context_skip/500ms/`
  ...
  - `results/figures/1.10-latent_dynamics_decoder/20260321_latent_dynamics_context_skip/training_curves.{png,pdf}`
  - `results/figures/1.10-latent_dynamics_decoder/optimization_progress.{png,pdf}`
- **产生时间**：2026-03-22
- **产生方式**：
  - `scripts/1.10-latent_dynamics_decoder/20260321_latent_dynamics_context_skip/run_latent_dynamics_context_skip_500ms.sh`
  - `scripts/analysis/neurohorizon/eval_phase1_v2.py`
  - `scripts/1.10-latent_dynamics_decoder/20260321_latent_dynamics_context_skip/collect_latent_dynamics_context_skip_results.py`
  - `scripts/1.10-latent_dynamics_decoder/plot_optimization_training_curves.py --module 20260321_latent_dynamics_context_skip`
  - `scripts/1.10-latent_dynamics_decoder/plot_optimization_progress.py`
- **实验目的**：
  - 验证“持续 encoder context 注入”是否能显著改善上一轮 `500ms valid fp-bps = 0.0048`
  - 若 `500ms` gate 仍无法恢复到首轮 `0.0904`，则终止当前 GRU context-skip 路径
- **实验配置**：
  - 模型：GRU latent dynamics decoder
  - 改动：`num_pool_tokens=4`, `pool_token_dim=32`, `state_dim=128`, `context_conditioning=True`, `context_dim=128`
  - 数据：Perich-Miller 10 sessions
  - 协议：continuous，obs=500ms，pred=500ms
  - 训练：300 epochs
  - 说明：本模块只做 `500ms gate`
- **主要结果**：
  ...
  | run | valid fp-bps | test fp-bps | valid R2 | test R2 | baseline_v2 | vs 上一轮 500ms | vs 首轮 500ms |
  |-----|--------------|-------------|----------|---------|-------------|-----------------|---------------|
  | `20260321_latent_dynamics_context_skip` | 0.0047 | 0.0048 | 0.1791 | 0.1790 | 0.1744 | -0.0001 | -0.0857 |

- **图表与解读**：
  - `training_curves.png`
    - best checkpoint 出现在 `epoch 69`，对应 best valid `fp-bps=0.0046`
    - 到 `epoch 299` 时 `val/fp_bps` 已回落到 `0.0009`，说明这轮同样不是“训练不够”，而是当前 GRU context-skip 结构没有形成稳定有效增益
  - `optimization_progress.png`
    - 新增的 `20260321_latent_dynamics_context_skip` 行只在 `500ms` 有值
    - 图上可以直接看到：它几乎与 `20260320_latent_dynamics_state_scaling` 重合，并且远低于首轮 `20260320_latent_dynamics_decoder` 的 `500ms=0.0904`
  - `latent_dynamics_context_skip_summary.json`
    - 固化了本轮 `500ms` gate 的 best checkpoint 与 formal valid 指标
    - 也保证了后续训练曲线和 `results.tsv` 更新不再依赖 screen log 手工整理
- **结果分析**：
  - 这轮 gate 给出的仍然是负结论：显式 persistent context 注入并没有把 `500ms` 从失败区间拉回来
  - 它几乎复制了上一轮 `state scaling` 的结果，说明当前问题并不只是“缺少持续条件输入”
  - 在当前 GRU latent dynamics 主线下，继续做局部结构微调的价值已经很低
- **备注**：
  - 本轮实现 commit：`4cbbbe0`
  - 该模块已标记为“已放弃”，不再扩展到 `250ms / 1000ms`
  - 下一轮 `1.10.x` 应直接转向更强的 dynamics backbone（优先 Mamba）

### 20260322_latent_dynamics_mamba_gate — implementation verify + smoke

- **存储路径**：
  - `results/logs/1.10-latent_dynamics_decoder/20260322_latent_dynamics_mamba_gate/500ms_smoke/`
- **产生时间**：2026-03-22
- **产生方式**：
  - `scripts/1.10-latent_dynamics_decoder/20260322_latent_dynamics_mamba_gate/verify_latent_dynamics_mamba_gate.py`
  - `examples/neurohorizon/train.py --config-name=train_1p10_latent_dynamics_mamba_500ms epochs=1 eval_epochs=1 batch_size=64 eval_batch_size=64 num_workers=0`
  - `scripts/analysis/neurohorizon/eval_phase1_v2.py --checkpoint-kind best --split valid`
- **实验目的**：
  - 验证官方 `mamba-ssm + causal-conv1d` backend 是否已打通最小训练与离线评估链路
  - 判断在正式 backend 下，`500ms` Mamba 路线是否能稳定跑完 1 epoch smoke
- **实验配置**：
  - 模型：Mamba latent dynamics decoder
  - backend：`mamba_ssm`
  - 数据：Perich-Miller 10 sessions
  - 协议：continuous，obs=500ms，pred=500ms
  - 训练：1 epoch smoke，`batch_size=64`
- **主要结果**：

  | run | backend | train loss | val loss | valid fp-bps | valid R2 |
  |-----|---------|------------|----------|--------------|----------|
  | `20260322_latent_dynamics_mamba_gate` smoke | `mamba_ssm` | 0.419 | 0.396 | -0.8290 | -0.0001 |

- **结果分析**：
  - 这一轮 smoke 已经证明：官方 `mamba-ssm` backend 在当前 `torch 2.10.0+cu128` 环境中可用，`forward/generate` 一致性、训练、checkpoint 与离线 valid eval 链路都已跑通
  - 与早先 fallback smoke 不同，本轮不再依赖 `transformers + mambapy`，并且 smoke 恢复到与正式 train config 对齐的 `batch_size=64`
  - 但数值层面并没有改善，`valid fp-bps` 仍停留在 `-0.8290`，说明 backbone 切换至少在 1 epoch smoke 上还没有带来立即收益
  - 因此 smoke 的意义是“环境已打通”，而不是“方向已验证有效”；真正判断仍要看正式 `500ms` gate
- **备注**：
  - verify 输出：`mamba_backend=mamba_ssm`, `output_shape=(2, 25, 6)`, `tf_vs_rollout_max_delta=0.000000`
  - 正式 `500ms` gate 的最终结果见下节

### 20260322_latent_dynamics_mamba_gate — 500ms formal gate

- **存储路径**：
  - `results/logs/1.10-latent_dynamics_decoder/20260322_latent_dynamics_mamba_gate/500ms/`
...
- **产生时间**：2026-03-22
- **产生方式**：
  - `scripts/1.10-latent_dynamics_decoder/20260322_latent_dynamics_mamba_gate/run_latent_dynamics_mamba_500ms.sh`
  - `scripts/analysis/neurohorizon/eval_phase1_v2.py`
  - `scripts/1.10-latent_dynamics_decoder/20260322_latent_dynamics_mamba_gate/collect_latent_dynamics_mamba_results.py`
  - `scripts/1.10-latent_dynamics_decoder/plot_optimization_training_curves.py --module 20260322_latent_dynamics_mamba_gate`
  - `scripts/1.10-latent_dynamics_decoder/plot_optimization_progress.py`
- **实验目的**：
  - 判断在官方 `mamba-ssm` backend 下，把 latent dynamics backbone 从 GRU 切到 Mamba 后，是否能把 `500ms` 从 `0.0047 / 0.0048` 的失败区间拉回去
  - 如果 `500ms valid fp-bps` 仍明显低于首轮 GRU `0.0904`，则本轮直接标记为失败，不扩展 `250ms / 1000ms`
- **实验配置**：
  - 模型：Mamba latent dynamics decoder
  - backend：`mamba_ssm`
  - 数据：Perich-Miller 10 sessions
  - 协议：continuous，obs=500ms，pred=500ms
  - 训练：300 epochs，`batch_size=64`
  - 说明：当前 decoder 的 `forward()` / `generate()` 仍共用同一路 latent rollout 逻辑，verify 中 `tf_vs_rollout_max_delta=0.000000`
- **主要结果**：

  | run | valid fp-bps | test fp-bps | valid R2 | test R2 | baseline_v2 | vs context skip | vs 首轮 GRU 500ms |
  |-----|--------------|-------------|----------|---------|-------------|-----------------|-------------------|
  | `20260322_latent_dynamics_mamba_gate` | 0.0056 | 0.0057 | 0.1793 | 0.1792 | 0.1744 | +0.0009 | -0.0848 |

- **图表与解读**：
  - `training_curves.png`
    - best checkpoint 出现在 `epoch 69`，对应 best valid `fp-bps=0.0056`
    - 到 `epoch 299` 时 `val/fp_bps` 已回落到 `0.0008`，说明这轮同样不是“训练不够”，而是当前 `pooled init state + prev_latent rollout` 结构本身上限很低
    - `val_loss` 和 `R2` 会稳定在与前两轮失败 gate 类似的区间，说明 Mamba 没有把模型带到一个新的优化阶段
  - `optimization_progress.png`
    - 新增的 `20260322_latent_dynamics_mamba_gate` 点位在 `500ms` 上略高于 `state scaling` 与 `context skip`
    - 但它仍几乎贴着失败区间，远低于首轮 GRU 的 `0.0904`，更远低于 `baseline_v2=0.1744`
    - 这张图给出的结论很直接：纯 backbone 替换没有扭转 `1.10.x` 最近三轮 `500ms` 的失败趋势
  - `latent_dynamics_mamba_summary.json`
    - 固化了 best checkpoint 的 continuous valid 结果与 per-bin `fp-bps`
    - 25 个 bin 的 `fp-bps` 全部是小幅正值，但整体只落在 `0.001–0.010` 区间，说明这轮不是“后半段突然崩掉”，而是整条预测窗都只是弱增益
- **结果分析**：
  - 这轮正式结果给出了一个非常清楚的双重结论：工程上成功，算法上失败
  - 工程上，官方 `mamba-ssm` 路线已经完全闭环，不再依赖任何 fallback；验证、训练、best-ckpt eval、summary 和图表链路都已跑通
  - 算法上，`500ms valid fp-bps=0.0056` 只比 `state scaling=0.0048` 和 `context skip=0.0047` 高出不到 `0.001`
  - 同时它相对首轮 GRU `500ms=0.0904` 仍低 `-0.0848`，相对 `baseline_v2=0.1744` 仍低 `-0.1688`
  - 由于该分支的 `forward()` / `generate()` 仍然一致，这轮负结果更像是 latent state 构造与 conditioning 方式本身不足，而不是 exposure bias 或单纯 GRU cell 太弱
  - 更具体地说，问题已经从“GRU 不够强”收敛成“单次 pooled init state 加纯 `prev_latent` autonomous rollout 这条主线不够强”
- **备注**：
  - 本轮实现 commit：`6f98786`
  - 结果已同步写入 `cc_todo/1.10-latent_dynamics_decoder/results.tsv`
  - 该模块已标记为“已放弃”，不再扩展到 `250ms / 1000ms`
