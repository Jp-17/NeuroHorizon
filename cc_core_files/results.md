...
| `repro_faithful_ndt2_250ms_optalign_acc16_e60` | 上游 `configure_optimizers()` + `accumulate=16` + 默认 50 epoch warmup | `-0.7006` | `-0.7156` | `0.1747` |
| `repro_faithful_ndt2_250ms_optalign_acc16_scaledwarmup_e60` | 同上，但 `lr_ramp_steps=5, lr_decay_steps=60` | `-0.6641` | `-0.6779` | `0.1746` |
...
2. **faithful NDT2 已经把代码链路跑真了，但当前 250ms 最优结果仍明显低于 legacy simplified baseline**。  
   这恰恰说明旧 `NDT2-like 0.1791` 不能被理解为“原始 NDT2 大致水平”。当前更保守、也更可信的说法是：faithful NDT2 在统一协议下暂时只接近 `fp-bps = 0`。
3. **更忠实地平移上游训练规程，并没有带来更好的当前 benchmark 结果。**  
   直接接入 `configure_optimizers()`、`accumulate_batches=16` 和 warmup-cosine 后，250ms held-out test 反而降到 `-0.7156`；把 warmup 缩到 `5/60` 后仍只有 `-0.6779`。这说明问题不只是“代码没对齐上游”，还包括**上游预训练时标与当前 benchmark 数据规模/目标存在失配**。
4. **1.8.3 还不能回填为“benchmark 完成”**。  
...
- `results/logs/phase1_benchmark_repro_faithful_ndt2_250ms_optalign_acc16_e60/results.json`
- `results/logs/phase1_benchmark_repro_faithful_ndt2_250ms_optalign_acc16_scaledwarmup_e60/results.json`
