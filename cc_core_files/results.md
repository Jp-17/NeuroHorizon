...
保存路径：`results/figures/phase1_v2/`（2026-03-25 已按 `legacy` 协议重新生成，避免与 evalfix 图混用）
...
2. **trial-aligned 仍然明显劣于 continuous**：三种窗口在 valid/test 上的 continuous `fp-bps` 仍全面优于 trial-aligned，且 500/1000ms trial 的 `fp-bps` 依然显著为负
