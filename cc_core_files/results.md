...
> 数据来源：`results/logs/phase1_v2_evalfix_{250ms,500ms,1000ms}_{cont,trial}/lightning_logs/version_0/eval_v2_valid_results.json` 与对应 `metrics.csv`
> 说明：为与 plan.md 1.3.4 的 canonical 5 图规范保持一致，此处以 `valid` split 生成专用图表包；`test` split 仍以本节表格与 JSON 作为正式 held-out 结果。

**Figure 1：fp-bps vs 预测窗口**（`01_fpbps_vs_window.png`）
- 目的：检查协议修正后 `fp-bps` 随预测窗口增长的整体衰减趋势是否仍成立。
- 结果：continuous 仍保持 `0.2164 -> 0.1823 -> 0.1374` 的缓慢下降，排序与 legacy 完全一致；trial-aligned 则保持负值（`-0.1983 / -0.2839 / -0.2152`），说明 evalfix 没有推翻主排序，只是把 gap 表达得更清楚。
...
- 结果：continuous 三组曲线全部保持正值，其中 `250ms-cont` 在 12 个 bin 上维持 `0.1735–0.2690`，最稳；`500ms-cont` 与 `1000ms-cont` 后段分别衰减到 `0.1206` 与 `0.0915`，说明长窗口退化主要出现在 horizon 末端。trial-aligned 在 500/1000ms 上则一路下探到 `-0.4119` / `-0.3579`，明显暴露出后段失稳。
...
**Figure 3：per-neuron PSTH-R2 热力图**（`03_psth_r2_heatmap.png`）
- 目的：用统一后的 `per_neuron_psth_r2` 观察各 target direction 下的 trial-averaged 预测质量，而不是继续沿用 legacy population-mean PSTH 口径。
- 结果：数值集中在 `0.49–0.68`，不再出现 legacy 版本中 “trial-aligned 一边倒更高” 的悖论。`250ms-cont` 在 direction 6 达到约 `0.68`，`500/1000ms` 的 continuous 与 trial-aligned 更接近，说明新的 per-neuron 口径显著重排了此前的叙事。
- 交叉引用：figure=`results/figures/phase1_v2_evalfix/03_psth_r2_heatmap.png`，中间结果=`results/logs/phase1_v2_evalfix_*/lightning_logs/version_0/eval_v2_valid_results.json`。

**Figure 4：continuous vs trial-aligned 对比柱状图**（`04_cont_vs_trial.png`）
- 目的：把 `fp-bps` 与 `R²` 放到同一张图里，检查两种训练方式在两个核心指标上的一致性。
- 结果：continuous 在三个窗口上都稳定优于 trial-aligned；其中 `500ms` 的 gap 最大，`fp-bps` 从 `0.1823` 对 `-0.2839`，`R²` 从 `0.2368` 对 `0.0602`。这表明 trial-aligned 的问题不是单一指标波动，而是 single-trial 连续预测质量整体不足。
- 交叉引用：figure=`results/figures/phase1_v2_evalfix/04_cont_vs_trial.png`，中间结果=`results/logs/phase1_v2_evalfix_*/lightning_logs/version_0/eval_v2_valid_results.json`。

**Figure 5：训练曲线**（`05_training_curves.png`）
- 目的：结合 `metrics.csv` 检查 6 个 run 的训练期收敛形态，判断 trial-aligned 的问题更像欠拟合还是过拟合。
- 结果：continuous 三组 best validation `fp-bps` 分别出现在 epoch `159 / 259 / 269`，之后只小幅回落；trial-aligned 的 `500ms / 1000ms` 则在 epoch `69 / 59` 后明显崩塌，到 epoch 299 时训练期 `val/fp_bps` 已掉到 `-1.2378 / -0.9621`。`250ms-trial` 的训练期 `val/fp_bps` 仍可保持正值，但其离线 sequential continuous eval 已是 `-0.1983`，说明训练期 logger 不能替代离线 evalfix JSON。
