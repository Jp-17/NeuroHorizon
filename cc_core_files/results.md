...
> 新协议（SequentialFixedWindowSampler + 全局累计 `fp-bps / R²` + `per_neuron_psth_r2`）已在 `phase1_v2_evalfix_*` 上重跑，结果、对比表与专用可视化已在本节后续补充。
...
> 说明：
> - `01-05` 已于 `2026-03-25` 重生为按训练模式拆分的两套图：原文件名保留 `continuous-only`，新增 `*_trial_aligned.png` 作为 `trial-aligned-only` companion 图
> - `04_cont_vs_trial*.png` 已从柱状图改为折线图
> - `06_benchmark_comparison.png` 改为 current benchmark reference 图：NeuroHorizon 使用 `phase1_v2_evalfix` held-out test，IBL-MtM / Neuroformer 改引 `1.8` 最新 faithful 记录；Legacy NDT2-like 仅保留 protocol-fix historical internal reference

**Figure 1A: fp-bps vs 预测窗口（continuous-only）** (`01_fpbps_vs_window.png`)
- 连续模式 fp-bps 随窗口增大线性下降（0.21 -> 0.17 -> 0.13）

**Figure 1B: fp-bps vs 预测窗口（trial-aligned-only）** (`01_fpbps_vs_window_trial_aligned.png`)
- Trial-aligned 模式 fp-bps 在三个窗口上均为负值，直观显示长期训练后的过拟合

**Figure 2A: Per-bin fp-bps 衰减（continuous-only）** (`02_perbin_fpbps_decay.png`)
- 连续 250ms：12 bin 相对稳定，无显著衰减
- 连续 500ms/1000ms：后段 bin 轻微衰减

**Figure 2B: Per-bin fp-bps 衰减（trial-aligned-only）** (`02_perbin_fpbps_decay_trial_aligned.png`)
- Trial-aligned 三个窗口的 per-bin fp-bps 整体位于零线以下，符合过拟合后的退化趋势

**Figure 3A: legacy PSTH-R2 热力图（continuous-only）** (`03_psth_r2_heatmap.png`)
- 8 方向 x 3 条件，展示 continuous-trained 三个窗口在 legacy population-mean PSTH 口径下的方向结构

**Figure 3B: legacy PSTH-R2 热力图（trial-aligned-only）** (`03_psth_r2_heatmap_trial_aligned.png`)
- 8 方向 x 3 条件，trial-trained 模型在该 legacy PSTH 口径下仍普遍高于 continuous-trained，这也是后续统一到 `per_neuron_psth_r2` 的动机之一

**Figure 4A: 窗口扫描折线图（continuous-only）** (`04_cont_vs_trial.png`)
- `fp-bps / R-squared` 双指标改为折线图，更适合展示三个预测窗口上的单调趋势

**Figure 4B: 窗口扫描折线图（trial-aligned-only）** (`04_cont_vs_trial_trial_aligned.png`)
- 同样采用折线图，便于单独观察 trial-trained 条件的退化曲线

**Figure 5A: 训练曲线（continuous-only）** (`05_training_curves.png`)
- 连续模型 val_loss 平稳下降，fp-bps 稳定上升

**Figure 5B: 训练曲线（trial-aligned-only）** (`05_training_curves_trial_aligned.png`)
- Trial 模型 val_loss 先降后升（过拟合转折约 epoch 50），fp-bps 急剧恶化

**Figure 6: latest benchmark reference 对照** (`06_benchmark_comparison.png`)
- NeuroHorizon：`phase1_v2_evalfix` held-out test（`250/500/1000ms = 0.2223 / 0.1740 / 0.1348`）
- IBL-MtM：faithful `250ms e10 / e50 / e300`
- Neuroformer：faithful rollout `50ms reference / 250ms canonical / 20260321 +session_cond`
- Legacy NDT2-like：`protocol-fix held-out test`，仅作历史内部参考
- 最右侧现为 250ms 聚焦 table：只比较 `NeuroHorizon 250ms`、`Neuroformer faithful 250ms`、`IBL e300 250ms`
- 该图仍属于 internal reference，不作为 formal benchmark summary
...
2. **trial-aligned 仍然明显劣于 continuous**：三种窗口在 valid/test 上的 continuous `fp-bps` 仍全面优于 trial-aligned，且 500/1000ms trial-aligned 条件的 `continuous fp-bps` 依然显著为负
...
#### evalfix 专用可视化补充（2026-03-25）

> 图表目录：`results/figures/phase1_v2_evalfix/`
> 生成脚本：`scripts/analysis/neurohorizon/phase1_v2_visualize.py --protocol evalfix --split valid --out-dir /root/autodl-tmp/NeuroHorizon/results/figures/phase1_v2_evalfix`
> 数据来源：
> - Figure 1–5：`results/logs/phase1_v2_evalfix_{250ms,500ms,1000ms}_{cont,trial}/lightning_logs/version_0/eval_v2_valid_results.json` 与对应 `metrics.csv`
> - Figure 6：`phase1_v2_evalfix_250ms_cont` held-out test + `Neuroformer faithful canonical 250ms` rollout test + `IBL e300 250ms` test
> 说明：按 2026-03-25 的补图调整，`phase1_v2_evalfix/` 目录只保留 `continuous-only` 图表；trial-trained 条件继续通过本节表格与 JSON 保留，不再单独生成 companion PNG。

**Figure 1：fp-bps vs 预测窗口**（`01_fpbps_vs_window.png`）
- 目的：只看 continuous 主线，检查协议修正后 `fp-bps` 随预测窗口增长的整体衰减趋势是否仍成立。
- 结果：continuous 仍保持 `0.2164 -> 0.1823 -> 0.1374` 的缓慢下降，排序与 legacy 完全一致，说明 evalfix 没有推翻 `250ms > 500ms > 1000ms` 的主结论。
- 交叉引用：figure=`results/figures/phase1_v2_evalfix/01_fpbps_vs_window.png`，中间结果=`results/logs/phase1_v2_evalfix_*/lightning_logs/version_0/eval_v2_valid_results.json`。

**Figure 2：per-bin fp-bps 衰减曲线**（`02_perbin_fpbps_decay.png`）
- 目的：定位长预测窗口内性能衰减发生在 horizon 的哪一段。
- 结果：三组 continuous 曲线全部保持正值，其中 `250ms-cont` 在 12 个 bin 上维持 `0.1735–0.2690`，最稳；`500ms-cont` 与 `1000ms-cont` 后段分别衰减到 `0.1206` 与 `0.0915`，说明长窗口退化主要出现在 horizon 末端。
- 交叉引用：figure=`results/figures/phase1_v2_evalfix/02_perbin_fpbps_decay.png`，中间结果=`results/logs/phase1_v2_evalfix_*/lightning_logs/version_0/eval_v2_valid_results.json`。

**Figure 3：continuous per-bin fp-bps 热力图**（`03_perbin_fpbps_heatmap.png`）
- 目的：把三种 continuous 预测窗口的 per-bin `fp-bps` 放在同一张 heatmap 里，直观看出哪一段 horizon 开始掉速。
- 结果：`250ms` 这一行整体颜色最均匀，说明短窗口最稳定；`500ms` 与 `1000ms` 从中后段开始明显变浅，且 `1000ms` 在远端 bins 接近 `0.09`，进一步确认长窗口劣化主要来自后段而不是前段。
- 交叉引用：figure=`results/figures/phase1_v2_evalfix/03_perbin_fpbps_heatmap.png`，中间结果=`results/logs/phase1_v2_evalfix_*/lightning_logs/version_0/eval_v2_valid_results.json`。

**Figure 4：continuous 主指标汇总**（`04_continuous_metric_summary.png`）
- 目的：把 continuous `fp-bps` 与 `R²` 两个主指标放到同一张图里，检查两条曲线的窗口趋势是否一致。
- 结果：两条曲线都随窗口增长单调下降，其中 `fp-bps` 的下降幅度更明显，说明长窗口退化在对数似然坐标下更敏感；`R²` 仍保持正值，说明模型并非完全失效，而是对长 horizon 的计数精度明显变差。
- 交叉引用：figure=`results/figures/phase1_v2_evalfix/04_continuous_metric_summary.png`，中间结果=`results/logs/phase1_v2_evalfix_*/lightning_logs/version_0/eval_v2_valid_results.json`。

**Figure 5：训练曲线**（`05_training_curves.png`）
- 目的：结合 `metrics.csv` 检查 3 个 continuous run 的训练期收敛形态，判断主线 baseline 的最优 epoch 和过拟合程度。
- 结果：三组 best validation `fp-bps` 分别出现在 epoch `159 / 259 / 269`，之后只小幅回落；这说明 continuous 主线整体是稳定收敛而不是早早崩塌，长窗口性能下降主要是任务难度上升，而不是训练过程失控。
- 交叉引用：figure=`results/figures/phase1_v2_evalfix/05_training_curves.png`，训练日志=`results/logs/phase1_v2_evalfix_*/lightning_logs/version_0/metrics.csv`，离线结果=`results/logs/phase1_v2_evalfix_*/lightning_logs/version_0/eval_v2_valid_results.json`。

**Figure 6：current benchmark 250ms 对照**（`06_benchmark_comparison.png`）
- 目的：在同一张图里对齐当前项目主 baseline 与最新 faithful benchmark 250ms 结果，便于快速判断 `NeuroHorizon` 与当前 benchmark 参考点的相对位置。
- 结果：`NeuroHorizon 250ms` 使用 held-out test `fp-bps = 0.2223`，仍高于 `IBL e300 250ms = 0.1938`，而 `Neuroformer faithful 250ms rollout = -8.0350` 仍显著为负。该图只作为 internal reference，不能外推成“正式 benchmark 已完成”的结论。
- 交叉引用：figure=`results/figures/phase1_v2_evalfix/06_benchmark_comparison.png`，NeuroHorizon=`results/logs/phase1_v2_evalfix_250ms_cont/lightning_logs/version_0/eval_v2_test_results.json`，Neuroformer=`results/logs/phase1_benchmark_repro_faithful_neuroformer_250ms_canonical_e50_aligned/formal_eval/eval_results.json`，IBL=`results/logs/phase1-autoregressive-1.8-benchmark_model/20260321_benchmark_ibl_e300_neuroformer_session_conditioning/ibl_mtm_combined_e300_aligned/results.json`。
...
- 更新后的内部参考图：`results/figures/phase1_v2/06_benchmark_comparison.png`
- 数据口径：
  - NeuroHorizon：`phase1_v2_evalfix` held-out test
  - Neuroformer：`1.8` 最新 faithful rollout reference（含 `50ms reference`、`250ms canonical`、`20260321 +session_cond`）
  - IBL-MtM：`1.8` 最新 faithful `e10 / e50 / e300`
  - Legacy NDT2-like：`protocol-fix held-out test`
- 注意：该图仍然是 internal reference，用于对齐当前 baseline 与最新 benchmark 记录，不作为正式 benchmark 总结图
...
### 1.8.3 e300 + session_conditioning benchmark visualization（2026-03-25）

- **存储路径（logs）**：
  - `results/logs/phase1-autoregressive-1.8-benchmark_model/20260321_benchmark_ibl_e300_neuroformer_session_conditioning/ibl_mtm_combined_e300_aligned/`
  - `results/logs/phase1-autoregressive-1.8-benchmark_model/20260321_benchmark_ibl_e300_neuroformer_session_conditioning/neuroformer_250ms_session_conditioning_e50/`
- **存储路径（figures）**：
  - `results/figures/phase1-autoregressive-1.8-benchmark_model/20260321_benchmark_ibl_e300_neuroformer_session_conditioning/`
- **来源脚本**：`scripts/phase1-autoregressive-1.8-benchmark_model/20260321_benchmark_ibl_e300_neuroformer_session_conditioning/gen_20260321_figures.py`
- **实验目的**：补充 plan.md 1.8.3 实验记录规范要求的 4 项额外可视化，同时将 training curves 规范化落盘到 figures 目录。
- **关键结果（IBL-MtM combined_e300_aligned）**：
  - best epoch: 282，formal valid/test fp-bps = 0.1938
  - per-bin fp-bps（250ms pred，12 bins x 20ms）：从 0.244（最近 bin）逐步下降至 0.140（最远 bin），存在轻微单调递减趋势
  - predicted_to_true_event_ratio = 11.1x，存在明显事件总量尺度偏差
- **关键结果（Neuroformer 250ms +session_cond）**：
  - best epoch: 39，test rollout fp-bps = -7.939，test true_past fp-bps = -8.658
  - 注：本轮 session conditioning 因 session_idx 传递路径 bug 实际未生效，结果仅可作为 canonical rerun 速度参考
- **可视化内容**：
  - `fp_bps_by_pred_window.png`：左图为 Neuroformer 不同预测窗口（50ms/250ms）的 rollout vs true_past fp-bps 对比；右图为 IBL-MtM epoch 增长趋势
  - `per_bin_fp_bps_decay.png`：左图（pred=250ms）展示 IBL-MtM e10/e50/e300 + NF canonical + NF +SC 共 7 条曲线；右图（pred=50ms）展示 NF reference rollout/true_past
  - `ibl_mtm_e300_config_timeline.png`：IBL-MtM e300 lr/weight_decay/eff_batch_size/warmup_progress，300 epochs
  - `neuroformer_sc_config_timeline.png`：NF +SC lr/weight_decay/eff_batch_size/warmup_progress，50 epochs
  - `benchmark_summary_table.png`：6 条 benchmark runs 汇总表（IBL-MtM e10/e50/e300 + NF reference-50ms/canonical-250ms/+SC-250ms）
- **结果分析**：
  - IBL-MtM per-bin 衰减曲线显示轻微的时序衰减，从最近 bin（0.244）到最远 bin（0.140），但整体仍保持正值，说明该模型对整个 250ms 预测窗口内的所有时间步均有正预测能力。
  - Neuroformer per-bin 衰减曲线无明显趋势，整个 250ms 窗口内各 bin 均在 -7.9 到 -8.1 附近波动，符合"生成路径系统性失调"而非"时序衰减"的失败模式。
  - IBL-MtM epoch 增长趋势：e10(-0.0017) -> e50(0.1345) -> e300(0.1938)，仍有上升空间但增量递减。
  - 交叉引用：JSON `results.json`，训练日志 `results/logs/...`，生成脚本 `scripts/.../gen_20260321_figures.py`，数据集 Perich-Miller 2018 10-session
