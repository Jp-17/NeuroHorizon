...
### 2026-03-22 Decoder Scheduled Sampling 250ms Screening

- **存储路径**：
  - `results/logs/phase1-autoregressive-1.9-module-optimization/20260320_decoder_scheduled_sampling/{setting}/250ms/`
...
- **产生时间**：2026-03-21 至 2026-03-22（最终汇总生成于 2026-03-22 17:34 CST）
- **产生方式**：
  ...
  - `python cc_todo/phase1-autoregressive/1.9-module-optimization/plot_optimization_progress.py`
- **实验目的**：在完整 `7 settings × 3 windows` 正式矩阵前，先按 `1.9.0` 连续采样协议完成 `250ms` setting 筛选，用 `rollout test fp-bps` 与 `teacher-forced - rollout gap` 决定后续 `500ms / 1000ms` 优先级
- **主要结果**：

  | rank | setting | rollout test | rollout valid | TF test | test gap | checkpoint epoch |
  |------|---------|--------------|---------------|---------|----------|------------------|
  | 1 | `decoder_ss_linear_0_to_050` | 0.2245 | 0.2210 | 0.2831 | 0.0586 | 159 |
  | 2 | `decoder_ss_fixed_025` | 0.2230 | 0.2189 | 0.2867 | 0.0638 | 159 |
  | 3 | `hybrid_mix035_plus_linear_050` | 0.2227 | 0.2210 | 0.2714 | 0.0487 | 159 |
  | 4 | `decoder_ss_linear_0_to_075` | 0.2193 | 0.2096 | 0.2744 | 0.0551 | 119 |
  | 5 | `decoder_ss_fixed_075` | 0.2190 | 0.2104 | 0.2566 | 0.0377 | 189 |
  | 6 | `memory_only_mix035` | 0.2176 | 0.2109 | 0.2794 | 0.0617 | 159 |
  | 7 | `decoder_ss_fixed_050` | 0.2173 | 0.2156 | 0.2741 | 0.0568 | 159 |

- **结果分析**：
  - `decoder_ss_linear_0_to_050` 当前是本轮 `250ms` screening 的最佳 `rollout test fp-bps` setting，已经超过 `memory_only_mix035` control（0.2176）和此前阶段性领先的 `decoder_ss_fixed_025`（0.2230）
  - `hybrid_mix035_plus_linear_050` 以 `0.2227` 排名第 3，但 `test gap=0.0487` 明显小于 `decoder_ss_fixed_025` 的 `0.0638`，说明 moderate decoder schedule 与 memory mix 的组合值得继续扩展
  - 更激进的高 rollout ratio（`fixed_075` / `linear_0_to_075`）没有在 `rollout test fp-bps` 上超过 moderate `0 -> 0.5`，但 `decoder_ss_fixed_075` 仍给出了最小 `test gap=0.0377`，表明更强 schedule 更偏向缩小 exposure gap，而不是提升峰值 score
  - 若后续继续扩展到 `500ms / 1000ms`，更合理的优先顺序是先跑 `decoder_ss_linear_0_to_050` 与 `hybrid_mix035_plus_linear_050`，并保留 `decoder_ss_fixed_075` 作为 low-gap reference
