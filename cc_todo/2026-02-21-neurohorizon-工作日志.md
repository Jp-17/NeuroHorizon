...
- **NeuroHorizon v1** (epoch 21/100, 21%):
  - train_loss: ~0.39
  - 验证指标：
    | Epoch | val_loss | val_bits_per_spike |
    |-------|----------|-------------------|
    ...
    | 14    | 0.3982   | -0.5729           |
    | 19    | 0.3895   | -0.5226           |
  - BPS 改善率从 +0.0435/epoch 降至 +0.0202/epoch
  - 预计 BPS=0 在 epoch ~45
- **NeuroHorizon v2 IBL** (epoch 20/100, 20%):
  - 使用 z-score 归一化参考特征
  - 验证指标：
  ...
  - **v2 vs v1 (epoch 19)**：v2 BPS -0.456 vs v1 -0.523 (+12.8% 改善)
  - fr_corr 0.814 = 模型能很好地预测发放率模式
  - R² 0.397 = 解释约 40% 的 spike count 变异
  - 归一化特征效果显著
