...
#### 训练进度监控 (最新更新 - Session 9)
- **NeuroHorizon v1** (epoch 25/100, 25%):
  - train_loss: ~0.38
  - 验证指标：
    | Epoch | val_loss | val_bits_per_spike |
    |-------|----------|-------------------|
    ...
    | 24    | 0.3841   | **-0.4939**       |
  - BPS 改善率继续放缓但仍在改善
- **NeuroHorizon v2 IBL** (epoch 57/100, 57%):
  - 使用 z-score 归一化参考特征
  - 验证指标：
    ...
    | 54    | 0.3718   | **-0.407** | **0.831** | -- |
  - **最佳 BPS：-0.407 (epoch 54)**，v2 vs v1 best: 17.5% 改善
  - 学习曲线接近收敛（val_loss 0.372-0.374 range）
  - fr_corr 0.831 稳定，R² ~0.40 稳定
- **POYO 基线** — 已终止 (epoch 108/200):
  - 所有 R² 为负，best epoch 39 (val_r2=-0.050)
  - GPU 已释放
- **GPU 状态**：
...
#### 训练进度更新 (Session 10 - 2026-02-22)
- **NeuroHorizon v1** (epoch 26/100, 26%):
  - train_loss: ~0.35-0.43（波动正常）
  - 验证指标完整历史：
    | Epoch | val_loss | val_bits_per_spike |
    |-------|----------|-------------------|
    | 4     | 0.4670   | -1.0075           |
    ...
    | 19    | 0.3895   | -0.5226           |
    | 24    | 0.3841   | -0.4939           |
  - BPS 改善率持续放缓：-0.057/5ep → -0.050/5ep → -0.029/5ep
  - 下次验证在 epoch 29
- **NeuroHorizon v2 IBL** (epoch 69/100, 69%):
  - 使用 z-score 归一化参考特征
  - 验证指标完整历史：
    | Epoch | val_loss | val_bps  | val_fr_corr | val_r2 |
    |-------|----------|----------|-------------|--------|
    | 4     | 0.4138   | -0.6789  | 0.733       | 0.320  |
    | 9     | 0.3937   | -0.5521  | 0.789       | 0.319  |
    | 14    | 0.3811   | -0.4667  | 0.808       | 0.392  |
    | 19    | 0.3788   | -0.4557  | 0.814       | 0.397  |
    | 24    | 0.3765   | -0.4463  | 0.819       | 0.396  |
    | 29    | 0.3751   | -0.4312  | 0.821       | 0.402  |
    | 34    | 0.3752   | -0.4260  | 0.828       | 0.401  |
    | 39    | 0.3742   | -0.4213  | 0.831       | 0.396  |
    | 44    | 0.3740   | -0.4270  | 0.831       | 0.398  |
    | 49    | 0.3744   | -0.4227  | 0.832       | 0.392  |
    | 54    | 0.3718   | **-0.4073** | 0.833   | **0.409** |
    | 59    | 0.3714   | -0.4090  | 0.832       | 0.408  |
    | 64    | 0.3718   | -0.4109  | 0.834       | 0.406  |
  - **模型已收敛**：epoch 54 后 BPS 在 -0.407~-0.411 范围内振荡
  - Best BPS: **-0.4073** (epoch 54)
  - Best val_loss: **0.3714** (epoch 59)
  - Best fr_corr: **0.834** (epoch 64)
  - Best R²: **0.409** (epoch 54)
  - v2 vs v1 对比（同 epoch 24）：BPS -0.446 vs -0.494 = **9.7% 改善**
  - v2 best vs v1 best（截至 epoch 24）：-0.407 vs -0.494 = **17.6% 改善**
  - 下次验证在 epoch 69
- **训练进度预估**：
  - v1: 74 epochs 剩余，~739s/epoch ≈ 15h
...
#### NH v1 训练持续更新 (epoch 35)
- 新增验证 epoch 34: val_loss=0.3809, val_bps=**-0.4621**
- 完整 BPS 趋势（每5 epoch）：
  -1.01 → -0.72 → -0.57 → -0.52 → -0.49 → -0.47 → **-0.46**
- 改善速率持续放缓：+0.28 → +0.15 → +0.05 → +0.03 → +0.02 → +0.01 per 5 epochs
- 预计最终（100 epochs）BPS 约 -0.42 ~ -0.40
- v1 vs v2 对比（epoch 34）：v1 -0.462 vs v2 -0.426 = v2 领先 7.8%
