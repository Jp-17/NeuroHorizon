...
- 已于 2026-03-17 14:38 通过 `screen` 后台启动，已于同日完成全部 6 组训练与 valid/test 离线评估
- 日志文件：`results/logs/phase1_v2_evalfix_rerun.log`
- 对比文件：`results/logs/phase1_v2_evalfix_comparison/comparison.{json,md}`

关键结果如下：

| 条件 | legacy valid fp-bps | evalfix valid fp-bps | evalfix test fp-bps |
|------|---------------------|----------------------|---------------------|
| 250ms-cont | 0.2115 | 0.2164 | 0.2223 |
| 250ms-trial | -0.1744 | -0.1983 | -0.1910 |
| 500ms-cont | 0.1744 | 0.1823 | 0.1740 |
| 500ms-trial | -0.2471 | -0.2839 | -0.2806 |
| 1000ms-cont | 0.1317 | 0.1374 | 0.1348 |
| 1000ms-trial | -0.1938 | -0.2152 | -0.2074 |

结论回填：

1. **建议 2（全局累计指标）与建议 3（deterministic eval sampler）已经落地，且不改变 v2 continuous baseline 的主排序**
2. **新的 `per_neuron_psth_r2` 使得旧的 population-mean `PSTH-R2` 叙述必须收紧**；此前 `results.md` 中关于 `PSTH-R2` 的结论不能再直接沿用
3. **trial-aligned 在 300 epochs 下仍然明显不优**；本轮修正后它在 valid/test continuous `fp-bps` 上依旧持续为负

## 13. 后续延续性改动计划（2026-03-18）

本节记录在第 11/12 部分 `evalfix` 体系之上的两项延续性改动，均不回退已经完成的评估协议修正：

1. **1.3.5：新增 IBL-MtM 风格 bps 对照指标**
   - 保持主 `fp-bps` 不变
   - 新增 `ibl_mtm_bps = per-neuron mean + eval-split null`
   - 仅补评估 `1.3.4 evalfix` 的 continuous 三个窗口，不重训 causal baseline
2. **1.3.6：baseline_v2 non-causal ablation**
   - 在 baseline_v2 训练方式上，仅将 `causal_decoder` 由 `true` 改为 `false`
   - 重跑 `250ms / 500ms / 1000ms` 三个 continuous 条件
   - 与 causal baseline 并排比较 `fp-bps` 与 `ibl_mtm_bps`
...
- `1.3.5` 已完成：
  - `eval_phase1_v2.py` continuous 输出已新增 `ibl_mtm_bps`
  - `250/500/1000ms` causal evalfix checkpoints 的 valid/test 已完成补评估
  - 对比表已生成：`results/logs/phase1_v2_metric_extension_comparison/comparison.{json,md}`
- `1.3.6` 已完成：
  - non-causal 三组配置与批量脚本已落地并跑完
  - 对比表已生成：`results/logs/phase1_v2_nocausal_comparison/comparison.{json,md}`
  - 结论是：causal 在 `250ms` 更好，`500ms` 基本持平，`1000ms` 则 non-causal 更好
