...
def run_training(name, overrides, epochs=50, log_dir_suffix=""):
    """Run a training with specific overrides."""
    log_dir = f"./logs/ablation_{name}{log_dir_suffix}"
    cmd = [
...
ABLATIONS = {
    "id_encoder": {
        "description": "IDEncoder vs random embeddings",
        "variants": {
            "with_idenc": {},  # Default: IDEncoder enabled
            "random_emb": {"model.ref_dim": 0},  # Random embeddings (no ref features)
        },
    },
    "horizon": {
