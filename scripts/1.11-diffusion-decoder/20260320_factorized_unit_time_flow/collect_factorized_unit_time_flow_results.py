#!/usr/bin/env python3
"""Collect formal Phase 1.11 factorized diffusion results and generate figures."""
...
MODULE_NAME = "20260320_factorized_unit_time_flow"
PREVIOUS_MODULE_NAME = "20260320_direct_count_flow_dit"
...
def previous_diffusion_eval_dir(root: Path, window: str) -> Path:
...
def summarize_window(root: Path, window: str) -> dict:
    ...
    previous_valid_path = previous_diffusion_eval_dir(root, window) / "eval_v2_valid_results.json"
    previous_test_path = previous_diffusion_eval_dir(root, window) / "eval_v2_test_results.json"
    ...
    previous_valid = load_json(previous_valid_path)
    previous_test = load_json(previous_test_path)
    ...
    return {
        ...
        "previous_diffusion_valid": previous_valid,
        "previous_diffusion_test": previous_test,
...
def collect_summary(root: Path) -> dict:
    ...
    return {
        ...
        "notes": (
            "Factorized unit-time flow tokens recover explicit unit structure and improve "
            "all three windows versus the first diffusion attempt, but continuous fp-bps "
            "remains materially below baseline_v2."
        ),
        "windows": windows,
    }
...
def plot_training_curves(summary: dict, figure_dir: Path) -> None:
    ...
    fig.suptitle("Phase 1.11 factorized unit-time diffusion training curves", fontsize=15, fontweight="bold")
    fig.text(
        0.5,
        0.02,
...
def plot_fp_bps_vs_window(summary: dict, figure_dir: Path) -> None:
    ...
    factorized_valid = [summary["windows"][window]["formal_valid"]["continuous"]["fp_bps"] for window in WINDOW_ORDER]
    factorized_test = [summary["windows"][window]["formal_test"]["continuous"]["fp_bps"] for window in WINDOW_ORDER]
    previous_test = [
        summary["windows"][window]["previous_diffusion_test"]["continuous"]["fp_bps"] for window in WINDOW_ORDER
    ]
    baseline_test = [summary["windows"][window]["baseline_test"]["continuous"]["fp_bps"] for window in WINDOW_ORDER]
    deltas_vs_previous = [summary["windows"][window]["delta_vs_previous_test_fp_bps"] for window in WINDOW_ORDER]
    deltas_vs_baseline = [summary["windows"][window]["delta_vs_baseline_test_fp_bps"] for window in WINDOW_ORDER]

    fig, axes = plt.subplots(1, 2, figsize=(13, 4.8))

    axes[0].plot(windows_ms, factorized_valid, marker="o", color="#1f78b4", linewidth=2.0, label="factorized valid")
    axes[0].plot(windows_ms, factorized_test, marker="o", color="#e31a1c", linewidth=2.0, label="factorized test")
    axes[0].plot(
        windows_ms,
        previous_test,
        marker="^",
        ...
        linestyle="--",
        linewidth=1.8,
        label="direct-count diffusion test",
    )
    ...
    positions = np.arange(len(windows_ms))
    width = 0.34
    axes[1].bar(
        positions - width / 2,
        deltas_vs_previous,
        width=width,
        color="#4daf4a",
        label="vs direct-count diffusion",
    )
    axes[1].bar(
        positions + width / 2,
        deltas_vs_baseline,
        width=width,
        color="#b2182b",
        label="vs baseline_v2",
    )
    axes[1].axhline(0.0, color="#333333", linewidth=1.2)
    axes[1].set_title("Factorized test deltas", fontsize=12, fontweight="bold")
    axes[1].set_xlabel("Prediction window (ms)")
    axes[1].set_ylabel("factorized test fp-bps delta")
    axes[1].set_xticks(positions, [str(value) for value in windows_ms])
    axes[1].grid(alpha=0.25, axis="y")
    axes[1].legend(frameon=False, fontsize=9)
    for idx, delta in enumerate(deltas_vs_previous):
        axes[1].text(
    ...
    for idx, delta in enumerate(deltas_vs_baseline):
        axes[1].text(
            positions[idx] + width / 2,
            delta,
            f"{delta:.3f}",
...
def plot_per_bin_fp_bps(summary: dict, figure_dir: Path) -> None:
    ...
    for window in WINDOW_ORDER:
        factorized = summary["windows"][window]["formal_test"]["continuous"]["per_bin_fp_bps"]
        previous = summary["windows"][window]["previous_diffusion_test"]["continuous"]["per_bin_fp_bps"]
        baseline = summary["windows"][window]["baseline_test"]["continuous"]["per_bin_fp_bps"]
        all_values.extend(factorized.values())
    ...
    for index, window in enumerate(WINDOW_ORDER):
        ax = axes[index]
        payload = summary["windows"][window]
        factorized = payload["formal_test"]["continuous"]["per_bin_fp_bps"]
        previous = payload["previous_diffusion_test"]["continuous"]["per_bin_fp_bps"]
        baseline = payload["baseline_test"]["continuous"]["per_bin_fp_bps"]
        x = list(range(len(factorized)))
        y_factorized = [factorized[str(bin_index)] for bin_index in x]
        y_previous = [previous[str(bin_index)] for bin_index in x]
        y_baseline = [baseline[str(bin_index)] for bin_index in x]

        ax.plot(x, y_factorized, color="#e31a1c", linewidth=2.0, label="factorized test")
        ax.plot(x, y_previous, color="#ff7f00", linestyle="--", linewidth=1.8, label="direct-count test")
...
def plot_summary_table(summary: dict, figure_dir: Path) -> None:
    ...
    ax.set_title("Phase 1.11 factorized diffusion formal summary", fontsize=14, fontweight="bold", pad=12)
    fig.tight_layout()
...
def write_summary_json(summary: dict, figure_dir: Path) -> None:
    output_path = figure_dir / "factorized_unit_time_flow_summary.json"
...
def update_results_tsv(root: Path, summary: dict) -> None:
    ...
    retained = [row for row in rows if row["module_name"] != "factorized_unit_time_flow"]
    for window in WINDOW_ORDER:
        payload = summary["windows"][window]
        ...
        retained.append(
            {
                "date": "2026-03-20",
                "module_name": "factorized_unit_time_flow",
                ...
                "notes": (
                    ...
                    f"delta_vs_prev_diffusion_test={payload['delta_vs_previous_test_fp_bps']:.4f}; "
