#!/usr/bin/env python3
"""Collect Phase 1.11 dense-history-cross diffusion results and generate figures."""
...
MODULE_NAME = "20260321_dense_history_cross_factorized_flow"
PREVIOUS_MODULE_NAME = "20260320_factorized_unit_time_flow"
ALL_WINDOWS = ["250ms", "500ms", "1000ms"]
...
def module_log_root(root: Path) -> Path:
    return root / "results" / "logs" / "1.11-diffusion-decoder" / MODULE_NAME


def module_figure_root(root: Path) -> Path:
...
def previous_eval_dir(root: Path, window: str) -> Path:
    return root / "results" / "logs" / "1.11-diffusion-decoder" / PREVIOUS_MODULE_NAME / window
...
def summarize_window(root: Path, window: str) -> dict:
    log_dir = module_log_root(root) / window
    metrics_path = log_dir / "lightning_logs" / "version_0" / "metrics.csv"
    valid_eval_path = log_dir / "eval_v2_valid_results.json"
    test_eval_path = log_dir / "eval_v2_test_results.json"

    metrics = load_metrics(metrics_path)
    valid_eval = load_json(valid_eval_path)
    test_eval = load_json(test_eval_path)
    baseline_valid = load_json(baseline_eval_dir(root, window) / "eval_v2_valid_results.json")
    baseline_test = load_json(baseline_eval_dir(root, window) / "eval_v2_test_results.json")
    previous_valid = load_json(previous_eval_dir(root, window) / "eval_v2_valid_results.json")
    previous_test = load_json(previous_eval_dir(root, window) / "eval_v2_test_results.json")
    train_cfg = TRAINING_CONFIGS[window]

    test_fp = test_eval["continuous"]["fp_bps"]
    baseline_test_fp = baseline_test["continuous"]["fp_bps"]
    previous_test_fp = previous_test["continuous"]["fp_bps"]
    ...
    return {
        ...
        "delta_vs_previous_test_fp_bps": test_fp - previous_test_fp,
        "delta_vs_baseline_test_fp_bps": test_fp - baseline_test_fp,
    }


def resolve_windows(root: Path, requested: list[str] | None) -> list[str]:
    if requested:
        return requested
    available = []
    for window in ALL_WINDOWS:
        test_json = module_log_root(root) / window / "eval_v2_test_results.json"
        if test_json.exists():
            available.append(window)
    if not available:
        raise FileNotFoundError("No eval_v2_test_results.json found for this module")
    return available


def collect_summary(root: Path, windows: list[str]) -> dict:
    payload = {window: summarize_window(root, window) for window in windows}
    return {
        "module": MODULE_NAME,
        "previous_module": PREVIOUS_MODULE_NAME,
        "windows_order": windows,
        "figures_dir": str(module_figure_root(root)),
        "log_dir": str(module_log_root(root)),
        "notes": (
            "Dense token-wise history cross-attention on top of the factorized unit/time flow baseline."
        ),
        "windows": payload,
    }
...
def plot_training_curves(summary: dict, figure_dir: Path) -> None:
    windows = summary["windows_order"]
    ncols = len(windows)
    fig, axes = plt.subplots(2, ncols, figsize=(5.7 * ncols, 9), sharex="col")
    if ncols == 1:
    ...
    for window in windows:
        ...
        loss_values.extend(point["val_loss"] for point in payload["val_curve"] if point["val_loss"] is not None)
        fp_values.extend(point["val_fp_bps"] for point in payload["val_curve"] if point["val_fp_bps"] is not None)
    ...
    for index, window in enumerate(windows):
        ...
        ax_loss.plot(
            [point["epoch"] for point in payload["train_curve"]],
            [point["train_loss"] for point in payload["train_curve"]],
            color="#d95f02",
            linewidth=2.0,
            label="train_loss",
        )
        ax_loss.plot(
            [point["epoch"] for point in payload["val_curve"]],
            [point["val_loss"] for point in payload["val_curve"]],
            color="#1b9e77",
            linewidth=2.0,
            label="val_loss",
        )
        ...
        x = [point["epoch"] for point in payload["val_curve"]]
        y = [point["val_fp_bps"] for point in payload["val_curve"]]
        ax_fp.plot(x, y, color="#1f78b4", linewidth=2.0, label="val/fp_bps")
        ax_fp.scatter(payload["best_val_fp_epoch"], payload["best_val_fp_bps"], color="#1f78b4", s=35, zorder=5)
        ax_fp.axhline(payload["formal_test"]["continuous"]["fp_bps"], color="#e31a1c", linewidth=1.5, label="module test")
        ax_fp.axhline(
    ...
    fig.suptitle("Phase 1.11 dense-history-cross training curves", fontsize=15, fontweight="bold")
    handles, labels = axes[1, 0].get_legend_handles_labels()
    fig.legend(handles, labels, loc="upper center", ncol=min(4, len(labels)), bbox_to_anchor=(0.5, 0.965), frameon=False)
...
def plot_fp_bps_vs_window(summary: dict, figure_dir: Path) -> None:
    windows = summary["windows_order"]
    windows_ms = [WINDOW_TO_MS[window] for window in windows]
    current_valid = [summary["windows"][window]["formal_valid"]["continuous"]["fp_bps"] for window in windows]
    current_test = [summary["windows"][window]["formal_test"]["continuous"]["fp_bps"] for window in windows]
    previous_test = [summary["windows"][window]["previous_test"]["continuous"]["fp_bps"] for window in windows]
    baseline_test = [summary["windows"][window]["baseline_test"]["continuous"]["fp_bps"] for window in windows]
    deltas_prev = [summary["windows"][window]["delta_vs_previous_test_fp_bps"] for window in windows]
    deltas_base = [summary["windows"][window]["delta_vs_baseline_test_fp_bps"] for window in windows]

    fig, axes = plt.subplots(1, 2, figsize=(13, 4.8))

    axes[0].plot(windows_ms, current_valid, marker="o", color="#1f78b4", linewidth=2.0, label="current valid")
    axes[0].plot(windows_ms, current_test, marker="o", color="#e31a1c", linewidth=2.0, label="current test")
    axes[0].plot(windows_ms, previous_test, marker="^", color="#ff7f00", linestyle="--", linewidth=1.8, label="prev test")
    ...
    width = 0.34
    axes[1].bar(positions - width / 2, deltas_prev, width=width, color="#4daf4a", label="vs previous")
    axes[1].bar(positions + width / 2, deltas_base, width=width, color="#b2182b", label="vs baseline")
    axes[1].axhline(0.0, color="#333333", linewidth=1.2)
    axes[1].set_title("Current test deltas", fontsize=12, fontweight="bold")
    axes[1].set_xlabel("Prediction window (ms)")
    axes[1].set_ylabel("current test fp-bps delta")
    axes[1].set_xticks(positions, [str(value) for value in windows_ms])
...
def plot_per_bin_fp_bps(summary: dict, figure_dir: Path) -> None:
    windows = summary["windows_order"]
    ncols = len(windows)
    fig, axes = plt.subplots(1, ncols, figsize=(5.7 * ncols, 4.8), sharey=True)
    if ncols == 1:
        axes = [axes]

    all_values: list[float] = []
    for window in windows:
        current = summary["windows"][window]["formal_test"]["continuous"]["per_bin_fp_bps"]
        previous = summary["windows"][window]["previous_test"]["continuous"]["per_bin_fp_bps"]
        baseline = summary["windows"][window]["baseline_test"]["continuous"]["per_bin_fp_bps"]
        all_values.extend(current.values())
        all_values.extend(previous.values())
    ...
    for index, window in enumerate(windows):
        ...
        ax.plot(x, [current[str(i)] for i in x], color="#e31a1c", linewidth=2.0, label="current test")
        ax.plot(x, [previous[str(i)] for i in x], color="#ff7f00", linestyle="--", linewidth=1.8, label="prev test")
        ax.plot(x, [baseline[str(i)] for i in x], color="#333333", linestyle="--", linewidth=1.8, label="baseline test")
    ...
    fig.legend(handles, labels, loc="upper center", ncol=min(3, len(labels)), bbox_to_anchor=(0.5, 1.02), frameon=False)
    fig.suptitle("Phase 1.11 per-bin fp-bps comparison", fontsize=15, fontweight="bold", y=1.08)
    fig.tight_layout()
...
def plot_summary_table(summary: dict, figure_dir: Path) -> None:
    ...
    rows = [
        [
            ...
            f"{summary['windows'][window]['delta_vs_previous_test_fp_bps']:.4f}",
    ...
    fig_height = 2.6 if len(rows) <= 2 else 2.8
    ...
    for (row, col), cell in table.get_celld().items():
        ...
        elif col in {3, 5, 6, 7, 8, 9}:
            cell.set_facecolor("#fbeaea")
    ax.set_title("Phase 1.11 dense-history-cross summary", fontsize=14, fontweight="bold", pad=12)
    fig.tight_layout()
...
def write_summary_json(summary: dict, figure_dir: Path) -> None:
    output_path = figure_dir / "dense_history_cross_factorized_flow_summary.json"
...
def update_results_tsv(root: Path, summary: dict) -> None:
    ...
    retained = [row for row in rows if row["module_name"] != "dense_history_cross_factorized_flow"]
    for window in summary["windows_order"]:
        payload = summary["windows"][window]
        retained.append(
            {
                "date": "2026-03-21",
                "module_name": "dense_history_cross_factorized_flow",
                ...
                "test_checkpoint": str(Path(payload["checkpoint_path"]).relative_to(root)),
                "notes": (
                    f"formal 300 epochs; best_ckpt_epoch={payload['checkpoint_epoch']}; "
                    f"delta_vs_prev_factorized_test={payload['delta_vs_previous_test_fp_bps']:.4f}; "
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--windows", nargs="*", choices=ALL_WINDOWS, default=None)
...
def main() -> None:
    args = parse_args()
    root = project_root()
    windows = resolve_windows(root, args.windows)
    figure_dir = module_figure_root(root)
    figure_dir.mkdir(parents=True, exist_ok=True)

    summary = collect_summary(root, windows)
