#!/usr/bin/env python3
"""Basic functional checks for the 1.10 Mamba latent-dynamics gate."""
...
def main() -> None:
    ...
    inputs = {
        ...
        "input_timestamps": torch.linspace(0.01, model.hist_window - 0.01, n_inputs)
        .unsqueeze(0)
        .repeat(batch_size, 1),
    ...
    if torch.isnan(forward_out).any() or torch.isnan(generate_out).any():
        raise AssertionError("NaN detected in latent dynamics Mamba outputs")
    ...
    print("latent_dynamics_mamba verification passed")
    print(f"mamba_backend={backend}")
